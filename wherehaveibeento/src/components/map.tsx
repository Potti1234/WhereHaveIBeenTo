...
export default function WorldMap(props : {user_id: string | null}) {
  ...
  const [visitedCountries, setVisitedCountries] = React.useState<Map<string, string>>(new Map<string, string>());
  const [countries, setCountries] = React.useState<Map<string, string>>(new Map<string, string>());
  ...
  const getCountryGeoJson = () => {
    supabase.storage.from('GeoJson').download(`world.geojson`).then((value) => {
       if (value.error) {
            return;
       }
  ...
  const onEachCountry = (country : any, layer: any) => {
    ...
    layer.bindPopup(
      countryName + "<br>" + "<a href='/explore/country/" + countryId + "'>View Information</a>"
    );

    layer.options.fillColor = visitedCountries.get(countryIso2) ? 'green' : 'red';
  };
  ...
  return (
    <div >
        <MapContainer
            ...
            {geoJson && scope==="countries" && <GeoJSON data={geoJson} onEachFeature={onEachCountry} />}
