...
export default function Page() {
    ...
    const fetchVisitedCountries = async (user_id : string) => {
        const { data, error } = await supabase.from('distinct_visited_countries')
        .select(`*`)
        .eq('user_id', user_id)
      ...
      const fetchVisitedStatesCount = async (user_id : string) => {
        const { count, error } = await supabase.from('distinct_visited_states')
      ...
      const fetchVisitedCitiesCount = async (user_id : string) => {
        const { count, error } = await supabase.from('visited_city')
        .select(`*`, { count: 'exact', head: true })
        .eq('user_id', user_id)
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="flex items-center justify-center">
                                <h1 className="text-4xl font-bold">{visitedCountries.length}</h1>
                            </div>
                        </CardContent>
                    </Card>
                    <Card>
