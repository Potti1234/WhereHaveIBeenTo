...
export default function Profile (props: { user_id: string | null }) {
  ...
  useEffect(() => {
    if (props.user_id) {
      ...
      fetchVisitedCountries(props.user_id)
      fetchVisitedStatesCount(props.user_id)
      fetchVisitedCitiesCount(props.user_id)
      return
    }
    ...
    setUserId(user.id)
    fetchVisitedCountries(user.id)
    fetchVisitedStatesCount(user.id)
    fetchVisitedCitiesCount(user.id)
  }, [])
          ...
          {selectedCountry === null && (
            <Card>
              ...
              <CardFooter className='border-t px-6 py-4'>
                <Button onClick={() => router.push('/map/' + userId)}>
                  View on map
                </Button>
              </CardFooter>
            </Card>
